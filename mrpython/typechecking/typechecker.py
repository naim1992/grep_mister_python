...
def type_check_Program(prog):
    ...
    #sixth step: check side effect
    for fun_name, fun_def in prog.functions.items():
        fun_def.impliciteSide_effect_check(ctx, prog.global_vars)
        if ctx.fatal_error:
            return ctx
...
# check side effect in functions
import side_effect_checking
FunctionDef.impliciteSide_effect_check = side_effect_checking.impliciteSide_effect_checking_err
...
if __name__ == '__main__':
    ...
    ctx = typecheck_from_file("../../test/side_effect_tests/list_err.py")
    print(repr(ctx))
